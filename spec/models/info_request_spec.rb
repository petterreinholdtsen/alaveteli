...
describe InfoRequest do
    ...
    describe 'when an instance is asked if it is old and unclassified' do
        ...
        before do
            ...
            @mock_comment_event = FactoryGirl.create(:info_request_event,
                                                     :created_at => Time.now - 23.days,
                                                     :event_type => 'comment',
                                                     :info_request => @info_request)
            @mock_incoming_message = FactoryGirl.create(:incoming_message,
                                                       :prominence => 'normal',
                                                       :info_request => @info_request)
            @mock_response_event = FactoryGirl.create(:info_request_event,
                                                      :info_request => @info_request,
        ...
        it 'should return false if its last response event occurred less than 21 days ago' do
            @mock_response_event.update_attribute(:created_at, Time.now - 20.days)
            @info_request.is_old_unclassified?.should be_false
        end
